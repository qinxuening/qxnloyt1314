...
class MobileController extends CommonController{
	...
	public function index(){
		...
		//print_r(S('List_Mobile_Cache_'.session('pid')));
	...
	public function update(){		
		...
		if($this->mobilemanager->CheckPid($Pid)){
			...
			if($this->mobilemanager->create($data)){
				if($this->mobilemanager->where(array("Pid" => $Pid, 'wUseID' => session('wUseID')))->save($data)){
					$this->mobilemanager->UpdateMobileInfo(array('McName' => I('post.McName')), I("get.id"));
				}
			}
			$this->linklist->where(array('McID' => $Pid))->delete();
