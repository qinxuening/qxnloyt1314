...
class a_product extends syController
{
	function __construct(){
		...
		if(($this->syArgs('a',1)=='add'||$this->syArgs('a',1)=='edit')&&$this->syArgs('run')==1){
			...
			foreach($v as $f){
				$ns='';$n=array();
				if($f['fieldstype']=='varchar' || $f['fieldstype']=='files' || $f['fieldstype']=='select'){$ns=$this->syArgs($f['fields'],4);}
	...
	function edit(){
		...
		if ($this->syArgs('run')==1){
			...
			//安卓生成xml所需数据
			$_clean ['version'] = $_POST['version'];
			$_clean ['banben'] = $_POST['banben'];
			$_clean ['ph'] = $_POST['ph'];
			$_clean ['about'] = $_POST['about'];
			$_clean ['gourl'] = "<![CDATA[".$_POST['gourl']."]]>";
			
			//苹果生成xml所需数据
			$_ping['banben'] = $_POST['ibanben'];
			$_ping['about'] = $_POST['about'];
			if ($_POST['tid'] == 40) {
				if ($_POST['id'] == 38) {$_ping['gour'] = 'https://www.loysmarthome.com/download/new/o-home.html';}
				if ($_POST['id'] == 37) {$_ping['gour'] = 'https://www.loysmarthome.com/download/o-home.html';}
			}
			...
			$newVerifier=$this->ClassADD->syVerifier($this->newrow1);
				if(false == $newVerifier){
					...
					if($edit_field){
						...
						//安卓写入文件
						if ($_POST['tid'] == 39) {
							if ($_POST['id'] == 36) {_set_xml_an('data/new/update.xml', $_clean);}
							if ($_POST['id'] == 35) {_set_xml_an('data/update.xml', $_clean);}
						}

						//苹果写入文件
						if ($_POST['tid'] == 40) {
							if ($_POST['id'] == 38) {_set_xml_pi('download/new/update.xml', $_ping);}
							if ($_POST['id'] == 37) {_set_xml_pi('download/update.xml', $_ping);}
						}
