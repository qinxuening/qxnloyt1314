...
VIVO_UIMIX.share = {
	url: function(type, content, url, image) {
		switch(type) {
			case 'weibo':
				return VIVO_UIMIX.share.weibo_url(content, url, image);
			case 'douban':
				return VIVO_UIMIX.share.douban_url(content, url, image);
			case 'renren':
				return VIVO_UIMIX.share.renren_url(content, url, image);
			case 'tencentweibo':
				return VIVO_UIMIX.share.tencentweibo_url(content, url, image);
			case 'qzone':
				return VIVO_UIMIX.share.qzone_url(content, url, image);
			case 'kaixin':
				return VIVO_UIMIX.share.kaixin_url(content, url, image);
			case '163weibo':
				return VIVO_UIMIX.share.t163weibo(content, url, image);
			case 'sohu':
				return VIVO_UIMIX.share.sohu(content, url, image);
			case 'msn':
				return VIVO_UIMIX.share.msn(content, url, image);
			default:
				return false;
		}
	},
	...
	renren_url: function(content, url, image) {
		...
		return 'http://share.renren.com/share/buttonshare.do?' + $.param(param);
	},

	qzone_url: function(content, url, image) {
		var param = {title: content};
		if (url) {
			param['url'] = url;
		}
