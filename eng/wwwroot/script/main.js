...
VIVO_UIMIX.main.news = {
	...
	newsShare : function(){
		...
		shareList.find("ul").css({width: 300 * shareTotal})
		.find("li a").click(function(){
			...
			if($(".news-content .news-views img").size()>0){
				// $(".news-content .news-views img").each(function(i,j){
				// 	imgURL+=$(j).attr("src")+"|";
				// 	if(i==$(".news-content .news-views img").size()-1) imgURL+=$(j).attr("src");
				// });
		...
		shareBtn.find("a").click(function(){
			...
			if($(this).hasClass("current")){
				var tim=0,
				th=$(this);
				shareList.stop().delay(500).animate({height:1},300,function(){
					$(this).css({display:"none"});
					th.removeClass("current");
				});
				shareList.find("ul li").each(function(i,j){
					var j=$(j);
					tim+=(shareTotal-(i+1)) * 50;
					$(this).css({"transition-delay": (shareTotal-(i+1)) * 50 +"ms"}).removeClass("start");
				});
			}else{
				var tim=0,
				th=$(this);
				...
				var t=setTimeout(function(){
					...
					shareList.find("ul li").each(function(i,j){
						var j=$(j);
						tim+=(shareTotal-(i+1)) * 50;
						$(this).css({"transition-delay": (i+1) * 50 +"ms"}).addClass("start");
					});
				},100);
			}
