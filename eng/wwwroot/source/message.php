...
{	
	...
	function add(){
		/*============================*/
		$url = "http://eng.loysmarthome.com/index.php?c=article&a=type&tid=26";
		$reg='/^\d{5,10}$/';
		if (!$_POST['mname']) {
			message('Name cannot be empty',$url);
		}else if (!$_POST['mcell']) {
			message('Mobile cannot be empty',$url);
		}else if ($_POST['mqq']&&!preg_match($reg, $_POST['mqq'])) {
			message('Incorrect Mobile',$url);
		}else if (strlen($_POST['body'])<10||strlen($_POST['body'])>200) {
			message('10-200 Letters',$url);
		}
		/*==============================*/
		...
		if(false == $newv){
			...
			message('提交成功，稍后客服处理您反馈信息！',$GLOBALS["WWW"]);
